            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredBooks.map((book) => (
                  <BookCard key={book.id} book={book} isOwner={book.authorId === user?.id} />
                ))}
              </div>
            ) : (
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredBooks
                  ...
                  .slice(0, 6)
                  .map((book) => (
                    <BookCard key={book.id} book={book} isOwner={book.authorId === user?.id} isNew />
                  ))}
              </div>
            ) : (
              <div className="text-center py-12">
                <BookOpen className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
                <h3 className="text-xl font-medium mb-2">Новых книг пока нет</h3>
                <p className="text-muted-foreground">Загляните позже, скоро здесь появятся новинки.</p>
              </div>
            )}
          </TabsContent>
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredBooks
                  ...
                  .slice(0, 6)
                  .map((book) => (
                    <BookCard key={book.id} book={book} isOwner={book.authorId === user?.id} isPopular />
                  ))}
              </div>
            ) : (
              <div className="text-center py-12">
                <BookOpen className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
                <h3 className="text-xl font-medium mb-2">Популярных книг пока нет</h3>
                <p className="text-muted-foreground">Читайте и оценивайте книги, чтобы они появились в этом разделе.</p>
              </div>
            )}
          </TabsContent>
        </Tabs>
    ...
    <div className="border rounded-lg overflow-hidden hover:shadow-md transition-shadow bg-white">
      ...
      <div className="p-4">
        ...
        <div className="text-xs text-muted-foreground mb-4">
          <span>Опубликовано: {formatDate(book.publishedAt || book.createdAt)}</span>
