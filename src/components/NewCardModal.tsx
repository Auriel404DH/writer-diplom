  ...
  return (
    <Dialog open={open} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>
            {editCard ? "Редактировать карточку" : "Новая карточка"}
          </DialogTitle>
        </DialogHeader>
              ...
              {cardTypes.map((type) => (
                <Button
                  ...
                  className={cn(
                    ...
                    cardType === type.id
                      ? type.bgClass + " border-" + type.id + "-200"
                      : "",
                  )}
            ...
            <div className="space-y-2">
              ...
              {Array.isArray(chapters) && chapters.length > 0 && (
                <div className="max-h-48 overflow-y-auto border border-neutral-200 rounded divide-y divide-neutral-200">
                  {chapters
                    .filter((chapter: any) => chapter.id !== chapterId)
                    .map((chapter: any) => (
                      <div
                        key={chapter.id}
                        className="flex items-center p-2 hover:bg-neutral-50"
                      >
                        <input
                          type="checkbox"
