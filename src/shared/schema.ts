...
export const cardChapters = pgTable(
  "card_chapters",
  {
...
export const insertUserSchema = createInsertSchema(users, {
  username: (schema) =>
    schema.min(3, "Имя пользователя должно содержать не менее 3 символов"),
  password: (schema) =>
    schema.min(6, "Пароль должен содержать не менее 6 символов"),
});
...
export const insertReviewSchema = createInsertSchema(reviews, {
  rating: (schema) =>
    schema.min(1, "Минимальная оценка - 1").max(5, "Максимальная оценка - 5"),
  content: (schema) =>
    schema.min(3, "Отзыв должен содержать не менее 3 символов"),
});
